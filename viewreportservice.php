...
<title>REPORT SERVICE SCREEN</title>
...
<form action="viewreportservice.php" method="POST">
...
$qry="select bill_service.sorderid,amount,cid,del_phone,gst,total,sbdate from bill_service,order_service where extract(month from sbdate)=$month and extract(year from sbdate)=$year order by sorderid";
$res=pg_query($conn,$qry);

echo "<h1>SERVICE REPORT INFORMATION</h1>";

echo "<table border=6px bordercolor=blue>";
echo "<tr><td>Service Order Id</td><td>Initial Amount</td><td>Customer Id</td><td>Delivary Phone</td><td>GST</td><td>Total</td><td>Service Bill Date</td>";

while($arr=pg_fetch_assoc($res))
{
echo "<tr>";
echo "<td>$arr[sorderid]</td><td>$arr[amount]</td><td>$arr[cid]</td><td>$arr[del_phone]</td><td>$arr[gst]</td><td>$arr[total]</td><td>$arr[sbdate]</td>";
echo "</tr>";
}
echo "</table>";
echo "<table border=6px bordercolor=blue>";

$qry1="select sum(total),avg(total) from bill_service where extract(month from sbdate)=$month and extract(year from sbdate)=$year";
